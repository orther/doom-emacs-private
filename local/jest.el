...
(def-package! mocha
  ...
  ;; https://github.com/scottaj/mocha.el/issues/3
  (defcustom mocha-jest-options "--coverage"
    "The path to the jest command to run."
    :type 'string
    :group 'mocha)
    ...
    (let ((target (if testname (concat " --testNamePattern \"" testname "\"") ""))
          ...
          (node-command
           (concat mocha-which-node
                   (if debug (concat " --debug-brk=" mocha-debug-port) "")))
          (options (concat mocha-jest-options (if debug " -t 21600000"))))
      (concat node-command " "
              mocha-jest-command " "
              options
              target
